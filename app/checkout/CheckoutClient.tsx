...
export default function CheckoutClient() {
  ...
  const selectedCourier = useMemo(() => couriers.find((c) => c.id === selectedCourierId) ?? null, [
    couriers,
    selectedCourierId,
  ]);
              ...
              <div className="bg-surface rounded-2xl border border-border overflow-hidden">
                <div className="">
                  <Table>
                    ...
                    <TableBody>
                      {items.map((it) => {
                        const imageSrc = normalizeImageSrc(it.product_image);
                        return (
                      ...
                      <SelectContent>
                        {addresses.map((a) => (
                          <SelectItem key={a.id} value={a.id} className="max-w-full">
                            <span className="block wrap-break-word whitespace-normal line-clamp-2" title={a.label}>
                              {a.label}
                            </span>
                          </SelectItem>
                        ))}
