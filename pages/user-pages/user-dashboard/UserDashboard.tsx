...
import { useUploadProfilePictureMutation, useUserProfileQuery } from "@/hooks/user/use-user-profile";
...
export default function AccountDashboard() {
  ...
  const selectedCount = useMemo(() => selectedOrders.length, [selectedOrders]);
  ...
  const handleUpload = async () => {
    ...
    const resolvedUserId = user?.id ?? readUserIdFromStorage();
    ...
    try {
      await uploadMutation.mutateAsync({ userId: resolvedUserId, file });
  ...
  if (!user) {
    const message = Number.isFinite(readUserIdFromStorage())
